...
''
' Retrieves a Profit and Loss report from the Ninja API for the selected date range.
'
' @method GetPnLReport
...
' 4. Sends the API request to the Ninja API using the configured WebRequest object.
...
'
' Note: This function uses the 'NinjaClient' property to execute the API request and the 'SelectReport' function to obtain the report date range.
''
Private Function GetPnLReport() As Dictionary
    ...
    Set ReportResponse = NinjaClient.Execute(ReportRequest)
    ...
    Else
        ...
        Err.Raise 11041 + vbObjectError, "NinjaAPICall.GetPnLReport", _
...
ApiCall_Cleanup:
    ...
    If Err.Number <> 0 Then
        ...
        WebHelpers.LogError auth_ErrorDescription, "NinjaAPICall.GetPnLReport", 11041 + vbObjectError
        Err.Raise 11041 + vbObjectError, "NinjaAPICall.GetPnLReport", auth_ErrorDescription
    End If
End Function
...
'
'        ' Log the error
'        WebHelpers.LogError auth_ErrorDescription, "NinjaAPICall.LoadReportToSheet", 11041 + vbObjectError
'        ' Raise the error for further handling
'        Err.Raise 11041 + vbObjectError, "NinjaAPICall.LoadReportToSheet", auth_ErrorDescription
'    End If
'End Sub
...
Public Sub GenerateReport_Click()
    ...
    MsgBox "Report successfully generated on sheet: " & vbNewLine & pLastOutputSheetName, vbInformation + vbOKOnly, "NinjaRMM Ticket Connector - Microsoft Outlook"
...
ApiCall_Cleanup:
    ...
    If Err.Number <> 0 Then
        ...
        ' Log the error
        WebHelpers.LogError auth_ErrorDescription, "NinjaAPICall.GenerateReport_Click", 11041 + vbObjectError
        ' Notify the user of the error
        MsgBox "ERROR:" & vbNewLine & vbNewLine & auth_ErrorDescription, vbCritical + vbOKOnly, "NinjaRMM Ticket Connector - Microsoft Outlook"
    End If
End Sub
